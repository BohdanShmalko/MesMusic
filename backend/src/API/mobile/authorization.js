...
const createUser = requestCreator(
  "post",
  "/mobile/createuser",
  async (req, res) => {
    ...
    const [_, shortDirPath] = dirPath.split("/fileStorrage");
    ...
    const rows = await database.addUser({
      ...userData,
      defaultPath: shortDirPath,
    });
    if (!rows.length) {
    ...
    await redisClient.setFromReq(req, { userId: rows[0].id });
...
const login = requestCreator("post", "/mobile/login", async (req, res) => {
  ...
  if (
    pwdObj.length &&
    passwordSecure.decript(pwdObj[0].truePassword) === password
  ) {
    const token = await redisClient.createToken();
    await redisClient.setFromToken(token, { userId: pwdObj[0].id });
    res.send({ token }, 200);
    return;
  }
...
const forgetPassword = requestCreator(
  "post",
  "/mobile/forgetpassword",
  (req, res) => {
