...
type GetBackgroundObjectType = (screen: ScreenNameType) => (state: StateType) => { color1: string, color2: string } | { image: { uri: string } }

// @ts-ignore
export const getBackgroundObject: GetBackgroundObjectType = (screen) => (state) => {
    const {firstMainColor, secondMainColor, pictureOnBackground} = state.settingsScreen.theme
